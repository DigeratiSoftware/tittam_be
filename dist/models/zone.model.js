...
const user_model_1 = require("./user.model"); // reuse counter
...
zoneSchema.pre("save", async function (next) {
    if (this.isNew) {
        try {
            const counterDoc = await user_model_1.Counter.findByIdAndUpdate({ _id: "zoneId" }, { $inc: { seq: 1 } }, { new: true, upsert: true });
